#
#  Runs NerTagger on given PostgreSQL collection that has named entity (NE) annotations.
#  Finds differences between old and new NE annotations.
#
...
from conf import create_ner_tagger
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Runs NerTagger on given PostgreSQL collection that has named entity (NE) annotations. "
        + "Finds differences between old and new NE annotations. "
        + "Outputs summarized statistics about differences, and writes all differences into a "
    ...
    parser.add_argument(
        "ner_layer",
        type=str,
        help='name of the NE layer to be compared against (the "old layer"). '
    ...
    parser.add_argument(
        ...
        help="name of the NE layer created during the re-annotation "
        + 'of the collection with NerTagger (the "new layer"). '
        "(default: 'new_named_entities')",
    )
    parser.add_argument(
        "--old_ne_attr",
    ...
    try:
        # Check layer names
        if args.ner_layer == args.new_ner_layer:
            log.error(
                "(!) Invalid layer names: ner_layer cannot be identical to new_ner_layer: {!r}".format(
                    args.ner_layer
                )
            )
            exit(1)
        ...
        if not collection.exists():
            ...
            ner_tagger, ner_input_layers_mapping = create_ner_tagger(
                args.ner_layer,
                collection,
                log,
                args.new_ner_layer,
            ...
            eval_layers = list(ner_input_layers_mapping.values()) + [args.ner_layer]
            data_iterator = None
            ...
            for key, text in data_iterator:
                ...
                flip_ner_input_layer_names(text, ner_input_layers_mapping)
                ner_tagger.tag(text)
