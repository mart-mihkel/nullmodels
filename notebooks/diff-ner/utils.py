...
def group_continuous_differences(diff_layer, in_layer_a, in_layer_b):
    ...
    for a, b in iterate_diff_conflicts(diff_layer, "span_status"):
        ...
        assert a_name != "__loc", (
            f"(!) Invalid layer name: {a_name}. Please use different name."
        )
        assert b_name != "__loc", (
            f"(!) Invalid layer name: {b_name}. Please use different name."
        )
    ...
    assert len(last_a_group_locs) == 0 or (
        len(last_a_group_locs) > 0 and len(last_b_group_locs) > 0
    )
    assert len(last_b_group_locs) == 0 or (
        len(last_a_group_locs) > 0 and len(last_b_group_locs) > 0
    )
    ...
    for mod_span in iterate_modified(diff_layer, "span_status"):
        assert len(mod_span.annotations) > 1, (
            f"(!) Unexpected number of annotations in a modified span {mod_span}"
        )
        # 2.1) Create separate spans for different layers
        sorted_annotations = sorted(
            mod_span.annotations, key=lambda x: x["input_layer_name"]
        )
        diff_spans = []
        for ann in sorted_annotations:
            if not diff_spans:
                ...
                new_ann = Annotation(
                    diff_spans[-1], **{k: ann[k] for k in ann.legal_attribute_names}
                )
                ...
                if prev_input_layer == cur_input_layer:
                    ...
                    diff_spans.append(
                        Span(base_span=mod_span.base_span, layer=diff_layer)
                    )
                    new_ann = Annotation(
                        diff_spans[-1], **{k: ann[k] for k in ann.legal_attribute_names}
                    )
                    diff_spans[-1].add_annotation(new_ann)
        assert len(diff_spans) == 2, (
            f"(!) Unexpected number of spans created in {diff_spans}"
        )
...
def remove_wordner_o_labels(grouped_diffs, label_attr="nertag", label_o_val="O"):
    ...
    for gid, group in enumerate(grouped_diffs):
        ...
        if len(set(group_label_values)) == 1 and group_label_values[0] == label_o_val:
...
def format_grouped_diffs_as_string(
    text_obj,
    text_cat,
    fname_stub,
    grouped_diffs,
    ...
    if format == "vertical":
        ...
        for gid, group in enumerate(grouped_diffs):
            ...
            before_a = "..." + _text_snippet(
                text_obj, conflict_loc[0] - N, conflict_loc[0]
            )
            before_b = "..." + _text_snippet(
                text_obj, conflict_loc[0] - N, conflict_loc[0]
            )
            if a_spans:
                before_a = "..." + _text_snippet(
                    text_obj, a_spans[0][0] - N, a_spans[0][0]
                )
            if b_spans:
                before_b = "..." + _text_snippet(
                    text_obj, b_spans[0][0] - N, b_spans[0][0]
                )
            ...
            for aid, (start, end) in enumerate(a_spans):
                ...
                output_a.append(
                    "{"
                    + _text_snippet(text_obj, start, end)
                    + "} /"
                    + annotation[label_attr]
                )
                last_span = (start, end)
            last_span = None
            for bid, (start, end) in enumerate(b_spans):
                ...
                last_span = (start, end)
            after_a = (
                _text_snippet(text_obj, conflict_loc[0], conflict_loc[1] + N) + "..."
            )
            after_b = (
                _text_snippet(text_obj, conflict_loc[0], conflict_loc[1] + N) + "..."
            )
            if a_spans:
                after_a = (
                    _text_snippet(text_obj, a_spans[-1][1], a_spans[-1][1] + N) + "..."
                )
            if b_spans:
                after_b = (
                    _text_snippet(text_obj, b_spans[-1][1], b_spans[-1][1] + N) + "..."
                )
            ...
            if text_cat:
                output_lines.append(
                    "  " + text_cat + "::" + fname_stub + "::" + str(gap_counter)
                )
            else:
...
class NerDiffFinder:
    ...
    def __init__(
        ...
        self.ner_diff_tagger = DiffTagger(
            layer_a=old_layer + self._flat_layer_suffix,
    ...
    def find_difference(self, text, fname, text_cat="", start_new_doc=True):
        ...
        """
        # Check input layers
        assert self.old_layer in text.layers, (
            f'(!) Input text is missing "{self.old_layer}" layer.'
        )
        assert self.new_layer in text.layers, (
            f'(!) Input text is missing "{self.new_layer}" layer.'
        )
        ...
        diff_layer = self.ner_diff_tagger.make_layer(
            text, {flat_a_name: old_layer_flat, flat_b_name: new_layer_flat}
        )
        # Group differences for better readability
        grouped_diffs = group_continuous_differences(
            diff_layer, flat_a_name, flat_b_name
        )
        formatted_str, new_gap_count = format_grouped_diffs_as_string(
            text,
            text_cat,
            fname,
            grouped_diffs,
        ...
        if start_new_doc:
            self.doc_counter += 1
        return diff_layer, formatted_str, total_diff_gaps
...
class NerDiffSummarizer:
    ...
    def record_from_diff_layer(
        self, layer_name, diff_layer, text_category, start_new_doc=True
    ):
    ...
    def get_diffs_summary_output(self, show_doc_count=True):
        ...
        for layer in sorted(self.diffs_counter.keys()):
            ...
            output.append("\n")
            diff_categories = [
                k for k in sorted(self.diffs_counter[layer].keys()) if k != "total"
            ]
            ...
            for category in diff_categories:
                ...
                if category == "total":
                    category = "TOTAL"
                output.append(
                    (" {:" + str(longest_cat_name + 1) + "}").format(category)
                )
                if show_doc_count:
                    output.append("|")
                ...
                second_layer_len = (
                    src["unchanged_spans"] + src["modified_spans"] + src["extra_spans"]
                )
                ...
                output.append(
                    " sim ratio: {} / {}  ({:.4f}) ".format(
                        src["unchanged_spans"] * 2, total_spans, ratio
                    )
                )
                ...
                output.append(
                    " only in {}: {} ({:.4f}%) ".format(
                        self.first_model, src["missing_spans"], missing_percent
                    )
                )
                ...
                output.append(
                    " only in {}: {} ({:.4f}%) ".format(
                        self.second_model, src["extra_spans"], extra_percent
                    )
                )
                output.append("\n")
            output.append("\n")
            ...
            for category in diff_categories:
                ...
                first_layer_len = (
                    src["unchanged_annotations"] + src["missing_annotations"]
                )
                second_layer_len = (
                    src["unchanged_annotations"] + src["extra_annotations"]
                )
                total_spans = first_layer_len + second_layer_len
                output.append("|")
                ...
                output.append(
                    " modified spans: {} / {} ({:.4f}) ".format(
                        src["modified_spans"], common_spans, ratio
                    )
                )
                output.append("|")
                # Ratio: https://docs.python.org/3.6/library/difflib.html#difflib.SequenceMatcher.ratio
                ...
                output.append(
                    " annotations sim ratio: {} / {} ({:.4f}) ".format(
                        src["unchanged_annotations"] * 2, total_spans, ratio
                    )
                )
                ...
                output.append(
                    " only in {}: {} ({:.4f}%) ".format(
                        self.first_model, src["missing_annotations"], missing_percent
                    )
                )
                ...
                output.append(
                    " only in {}: {} ({:.4f}%) ".format(
                        self.second_model, src["extra_annotations"], extra_percent
                    )
                )
                output.append("\n")
            output.append("\n")
